...
var gemischterSpanischerSatz: string[]; //variable für das mischen der wörter im spanischen satz, wird später genutzt
var satz = null; //leere variable, wird später genutzt

interface Satz { //hier werden die eigenschaften der beiden arrays satzSpanisch und satzDeutsch deklariert
    satzSpanisch: string[]; //array mit dem spanischen satz vom typ string
    satzDeutsch: string[]; //array mit dem deutschen satz vom typ string
}

var saetze: Satz[] = [ //Anfang von den Arrays, die die deutschen und spanischen Sätze enthalten
    {
        satzSpanisch: "El pájaro primitivo atrapa al gusano.".split(" "), //.split spaltet das array so auf, dass die wörter einzeln gelesen werden; wörter werden zu "substrings"
...
function shuffleArray(array): void { //das array wird durch eine mischung zufällig ausgegeben
    for (var i: number = array.length - 1; i > 0; i--) { //der variable i wird die arraylänge zugewiesen. sie wird solange abgespielt bis i gleich 0 ist
        var j: number = Math.floor(Math.random() * (i + 1)); //hier erhält die variable j eine zufällige stelle
...
window.addEventListener("load", function () {
    ...
    var schwierig: HTMLElement = document.getElementById("schwierigkeitslevel"); //das ist die variable für die schwierigkeitsstufen leicht, mittel und schwer. es wird auf das jeweilige html element zugegriffen
    ...
    document.getElementById("leicht").onclick = function () { //wenn man auf button mit id=leicht klickt...
    ...
    document.getElementById("mittel").onclick = function () { //wenn man auf den button mit id=mittel klickt...
    ...
    document.getElementById("schwer").onclick = function () { //wenn man auf den button mit id=schwer klickt...
    ...
    document.getElementById("neustart").onclick = function () { //wenn auf den button mit der id neustart geklickt wird, kommt die funktion neustart, die später definiert wird
        neustart();
    };

    function wortKlick(wort: string) { // neue funktion nach dam anklicken der wörter wird deklariert
        ...
        if (wort == saetze[satz].satzSpanisch[richtigeWörter]) { //wenn das wort mit der position im satzSpanisch Array übereinstimmt
            ...
            for (var i = 0; i < gemischterSpanischerSatz.length; i++) { //schleife: variable i wird auf 0 gesetzt und solange i kleiner als die länge des gemischten satzes ist, wird i hochgezählt
                let wort: HTMLSpanElement = document.createElement("span"); //let=wort wird erstellt und ihr variable span (zeile 126) zugewiesen
                ...
                wort.addEventListener = function () { //wort wird angeklickt, was die funktion wortKlick auslöst
                    wortKlick(wort.innerHTML);
                };
                document.getElementById("satzSpanisch").appendChild(wort); //wort wird an satzSpanisch angefügt
            }

            if (richtigeWörter == saetze[satz].satzSpanisch.length - 1) neuerSatz(); //Wenn Länge spanischerSatz gleich richtigeWörter ist, dann entsteht neuer Satz
        } else { //ansonsten...

            alert("Falsches Wort"); //...soll ein alert mit dem inhalt falsches wort erscheinen
            if (punkte > 0) punkte = punkte - 1; //...und es wird ein punkt abgezogen, solange die punktzahl eins oder größer ist. sollte sie 0 sein, wird kein punkt abgezogen
        }
    ...
    function neuerSatz() { //funktion für einen neuen satz
        ...
        if (satz == null) satz = 0; //wenn es keine Sätze im Array mehr gibt, wird die anzahl der sätze auf null gesetzt
        else satz = satz + 1; //sollte die anzahl der sätze 5/10/15 noch nicht erreicht sein, erscheint der nächste satz
        ...
        var spanisch: string[] = [...saetze[satz].satzSpanisch]; //ein neuer satz aus dem array satzSpanisch wird ausgewählt
        ...
        for (var i = 0; i < spanisch.length; i++) { //solange i kleiner ist als die länge vom spanischen satz, wird i hochgezählt
            ...
            wort.onclick = function () { //durch das anklicken des kreierten wortes wird die funktion wortKlick ausgelöst
                wortKlick(wort.innerHTML);
            };
    ...
    function neustart() { //wenn ich nach dem Beenden auf den Letzten Satz klicke...
